...
router.get('/api/v1/paidMembers', isOfficer, async function (req, res, next) {
  const getPayingMembersQuery = db.members
    .select(db.members.firstName, db.members.lastName, db.members.email, db.payments.timestamp)
    .from(db.members.join(db.payments).on(db.members.id.equals(db.payments.memberId)))
    .where(db.payments.timestamp.gte('2019-09-15').and(db.payments.timestamp.lt('2019-12-15')))
    .order(db.members.firstName)
    .toQuery();
